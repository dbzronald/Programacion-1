...
public class MainWindow extends JFrame {
	...
	private static ComplejoLacteo complejo;
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MainWindow frame = new MainWindow(complejo);
	 ...
	 * Create the frame.
	 */
	public MainWindow(final ComplejoLacteo complejo) {
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 450, 300);
		
		ComplejoLacteo.getInstance().loadComp(complejo);
		...
		addWindowListener(new WindowAdapter()
        {
            ...
            {	
                ComplejoLacteo.getInstance().saveComp(ComplejoLacteo.getInstance());;
                e.getWindow().dispose();
            }
        });
	}
